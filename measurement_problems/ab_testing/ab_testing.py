...
#buradan h1 in sağlanmadığını görüyoruz
...
#varyansaların homojen olmadığını görüyoruz
...
#!!!!!!!!!!!!ihtiyaca ve çıkan sonuçlara göre bunları uyguluyoruz
...
#varsıyımlar sağlanmadığı için bu testi uguladım ve sonuç olarka # p-value < değilse 0.05 H0 REDDEDILEMEZ. sonucu sağmadı yani varsayımımızda
#kayda değer bir neden olmadğını gösterdi
...
# H0: Normal dağılım varsayımı sağlanmaktadır.
# H1:..sağlanmamaktadır
test_stat, pvalue = shapiro(df.loc[df["sex"] == "female", "age"].dropna()) # H0 ı sağlayamadığımız için reddedttik
print('Test Stat = %.4f, p-value = %.4f' % (test_stat, pvalue))
...
print('Test Stat = %.4f, p-value = %.4f' % (test_stat, pvalue)) #varyanslar homıjendir H0 ı reddedemedik

# 90 280
test_stat, pvalue = mannwhitneyu(df.loc[df["sex"] == "female", "age"].dropna(),
                                 df.loc[df["sex"] == "male", "age"].dropna())

print('Test Stat = %.4f, p-value = %.4f' % (test_stat, pvalue)) # H0 ı reddedmedik çünkü p<0.05 ten bu yüzden bu varsayıma göre kayda değer bir fark var
...
# Hipotez (H0: M1 = M2) # H0 ı redettik çünkü p < 0.05 ten küçük
